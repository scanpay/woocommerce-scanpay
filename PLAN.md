...
**File:** `src/public/generate-payment-link.php`, `wc_scanpay_process_payment()`
— the fall-through path below the `$subid` branch (`:171` onward).

WooCommerce Subscriptions runs "change payment method" as
`process_payment( $subscription->get_id() )`, so the `$wco` our builder holds is
the **subscription**, not an order
(`includes/class-wc-subscriptions-change-payment-gateway.php:339`). WCS makes
that request free by filtering the total to zero — `maybe_zero_total()` on
`woocommerce_subscription_get_total`, registered in `init()` at `:33` — and its
own comment on the call site reads "Process payment for the new method (with a
$0 order total)".

That filter is a **view-context** filter. `WC_Data::get_prop()` applies
`{hook_prefix}{prop}` only when `'view' === $context`, and this builder reads
`$wco->get_total( 'edit' )` at `:204` and sums `get_line_total()` over
`$wco->get_items()` above it. So the payload is built from the subscription's
real recurring total.

The `$subid` branch above already knows all of this: it tests
`wcs_scanpay_is_payment_method_change()` at `:123`, sends the customer to
`renew()`, adds no thank-you wait and writes no metadata — with the reason
spelled out at `:118-122`, that a key on a subscription is copied onto every
future renewal order by `WC_Subscriptions_Data_Copier`. None of that reasoning
was ever applied to the path below it, and the path below it is the one a
subscription reaches when it has **no** `_scanpay_subid` — that is, every
subscription being moved to Scanpay *from another gateway*, which is what
"change payment method" is normally used for.

What that customer gets today:

1. A payment window for the **full recurring amount** (`$data['items']`, and an
   `orderid` that is the subscription id).
2. `_scanpay_payid`, `_scanpay_payid_time`, `_scanpay_shopid` and
   `_scanpay_complete` written onto the subscription (`:258-264`) — and from
   there copied onto every renewal order WCS creates afterwards. With
   `wcs_complete_initial` on, that copied `_scanpay_complete` is a stored `'1'`,
   which `WC_Scanpay_Sync::sync()` (`:324-341`) obeys by forcing every future
   renewal to `completed`, against the merchant's `wcs_complete_renewal` setting.
3. `scanpay_thankyou` / `scanpay_type` / `scanpay_ref` appended to the
   WCS-filtered My Account return URL (`:246-253`), which carries no `key`, so
   the wait never runs and the args are litter.

**The fix.** Give the method change its own early return, directly below the
`$subid` block and above the item build, mirroring the branch that already
handles it:

1. `if ( wcs_scanpay_is_payment_method_change() ) { … }` — inside it, set
   `$data['subscriber'] = [ 'ref' => wc_scanpay_subref( $oid, $wco ) ]`
   (`wc_scanpay_subref()`'s first branch is written for exactly this and returns
   `'wcs[]' . $oid`), call `$client->new_url( $data )` inside the same
   `try`/`catch ( Exception )` shape the file already uses, and return the
   `result`/`redirect` pair.
2. Send **no** `items`, append **no** query args to `$data['successurl']`, and
   write **no** meta. Leave `$data['autocapture']` and `$data['orderid']` as
   computed above; with no items there is nothing to capture, the backend
   discards the `orderid` (see below), and touching either only widens the diff.
3. Comment it the way `:118-122` is commented: why a subscription's total lies in
   the `edit` context, why meta written here rides the data copier onto every
   renewal, and — the part that must survive this plan being deleted — that
   `/v1/new` with a `subscriber.ref` and no `items` registers a card and creates
...
`orderid` is discarded rather than stored, so it never comes back through the
seq. That is why step 2 can leave `orderid` alone — there is no transaction for
sync to resolve against the subscription id, and therefore no `scanpay_meta` row
and no shopid-mismatch log. The link this branch returns is a card registration.

**Do not** switch `:204` to `get_total( 'view' )` to fix this. That would let a
third-party filter decide the amount on every ordinary order too, and the two
other order-total readers in the tree (`class-wc-scanpay-sync.php:477`, the
parent order of a free-trial subscription; `class-wcs-scanpay-charge.php:139`,
the renewal order) deliberately use `edit` for the same reason. Neither reads a
subscription's total — this builder is the only place that ever does, and only
because WCS hands it a subscription in place of an order.

### Verify

- Read `includes/class-wc-subscriptions-change-payment-gateway.php:33`,
  `:335-339` and `:618-629`, and `WC_Data::get_prop()` (`abstract-wc-data.php:952`,
  the filter at `:959`). State in `HANDOFF.md` that the zeroing is view-context
  only, quoting both.
- Trace the fall-through with `$subid === 0` and confirm, line by line, which of
  `items`, the successurl args and the four meta writes the new branch skips.
- Confirm `wc_scanpay_subref()` still returns `'wcs[]' . $oid` for this request
  without the item loop having run — its method-change branch is the first
  statement in the function and reads nothing the loop sets.
- Grep `_scanpay_` against `DEFAULT_EXCLUDED_META_KEYS`
  (`class-wc-subscriptions-data-copier.php:20-44`) and report that none of our
  keys is excluded from the copy.
- Confirm nothing but the three gateways' `process_payment()` reaches
  `wc_scanpay_process_payment()`.

### Handoff

- On a real shop with WCS: take a subscription paid by another gateway, use
  My Account → Change payment method → Scanpay, and confirm the payment window
  asks for **nothing** (a card registration, not a full renewal charge), that the
  subscription ends up linked (`_scanpay_subid` written by sync's
  `subscriber()`), and that no `_scanpay_complete` lands on the subscription.
- Then let one renewal run and confirm the renewal order carries no copied
  `_scanpay_complete` and completes (or does not) purely per
  `wcs_complete_renewal`.
- Confirm the customer lands back on My Account with a clean URL.
- Confirm no `scanpay_meta` row appears against the subscription id, and no
  "shopid mismatch" line in the log — the settled answer above says there is no
  transaction to drain, so a row there would mean the backend contract changed.

## Task B — The terms checkbox can be enforced without ever being rendered

**Files:** `src/public/wcs-scanpay-checkout-terms.php`;
`src/woocommerce-scanpay.php`, `wcs_scanpay_validate_terms()` at `:224-236`.

`AGENTS.md` states the invariant: one predicate drives both rendering and
validation, "so the checkbox can never be enforced without having been rendered,
or the reverse". The predicate is shared, but the two hooks are not equally
reachable. Validation runs from `woocommerce_after_checkout_validation`, which
`WC_Checkout` fires unconditionally. Rendering runs from
`woocommerce_checkout_after_terms_and_conditions`, and that hook lives at
`templates/checkout/terms.php:39` — inside a template whose entire body is
wrapped in
...
So on a store that returns false from that filter, or whose theme overrides
`checkout/terms.php` or `checkout/payment.php` without the hook, nothing renders
and `$_POST['wcssp-terms']` is always empty — while the validator keeps
rejecting.
Every classic checkout with a subscription in the cart then fails with "You must
accept the subscription terms to complete your purchase.", with nothing on the
page to tick. The customer cannot buy, and no gateway of ours has to be involved.

WooCommerce solves exactly this for its own checkbox, in the same template: it
emits `<input type="hidden" name="terms-field" value="1" />` (`:33`) and enforces
only when that marker came back — `! empty( $data['terms-field'] )`
(`includes/class-wc-checkout.php:794`, `:981`).

**The fix.** Make the render site the source of truth, the same way:

1. In `wcs-scanpay-checkout-terms.php`, emit
   `<input type="hidden" name="wcssp-terms-field" value="1">` next to the
   checkbox, inside the same `'' !== $url` branch.
2. In `wcs_scanpay_validate_terms()`, add `&& ! empty( $_POST['wcssp-terms-field'] )`
   to the condition that adds the error, under the existing
   `phpcs:ignore` for the checkout nonce.
3. Comment both halves with what they buy and what they cost: a crafted POST that
   omits the marker now skips the check, which is the same trade WooCommerce
   makes for its own terms, and the Blocks path is unaffected — there the
   checkbox is a forced block and `wcs_scanpay_blocks_validate_terms()` stays
   strict.

**Do not** touch the Blocks validator, and do not re-scope either half to a
gateway. The consent is cart-level; this task only stops it being demanded on a
checkout that never showed it.

### Verify

- Quote `templates/checkout/terms.php:11` and `:39` and state that our render
  hook is inside the filtered block.
- Quote WooCommerce's own marker and its `! empty( $data['terms-field'] )` test.
- Confirm the pay-page exclusion in `wcs-scanpay-checkout-terms.php:12` still
  behaves: no marker is emitted there, so the validator (which never runs on that
  endpoint anyway) cannot fire.
- Confirm the Blocks path posts nothing named `wcssp-terms-field` and is
  untouched.
- Record the new msgid situation: none. This task adds no translated string.

### Handoff

- On a shop: `add_filter( 'woocommerce_checkout_show_terms', '__return_false' )`,
  put a subscription in the cart with `wcs_terms` set to a published page, and
  confirm classic checkout now completes instead of failing with an
  un-tickable requirement.
- Remove the filter and confirm the checkbox is still enforced: submit with it
  unticked and see the error.

## Task C — A renewal charge must record the shop it ran under

**Files:** `src/library/class-wcs-scanpay-charge.php` (`charge()`, the meta write
at `:308-313`), and the `$paid_renewal` block of
`src/public/generate-payment-link.php:157-163` **as task A left it**.

`scheduled_charge()` deliberately charges an order whose `_scanpay_shopid` is
absent (`:115-116`): it logs a warning and proceeds, because a 1.x-migrated store
can legitimately have no stamp — sync only writes it for subscriptions resolved
from a `wcs[]` ref, which 1.x never wrote.

Both downstream readers treat an absent stamp as *another shop's order*:

- `WC_Scanpay_Sync::sync():258-261` — `(int) $wco->get_meta( … ) !== $this->shopid`
  is true for `0`, so the drained charge is logged as a "shopid mismatch" and
  dropped. No `transaction_id`, no `payment_complete()`, **no order note**.
- `WC_Scanpay_Capture::capture():54-58` — throws `ShopID mismatch … order has 0`,
  so a manual completion parks the order on hold instead of capturing.

The money has already moved by then. And every subsequent WCS retry hits the
authoritative guard at `class-wcs-scanpay-charge.php:283-294` — the
`scanpay_meta` row *was* written by sync before the mismatch check — and returns
`"charge skipped … already paid"` **without writing a status**, so WCS never sees
a failure either. The renewal order stays `pending`, the subscription stays
`on-hold`, and the customer has been charged.

The same file already knows where this belongs: `charge()` persists the
completion intent immediately before the API call, "durable, because the money
moves next: if the process dies here, sync still knows what this attempt asked
for" (`:305-313`). Which shop it asked under is the other half of that record.

**The fix.**

1. In `charge()`, next to the `WC_SCANPAY_URI_COMPLETE` write and before
   `$this->client->charge()`, stamp the shop when the order has none:
   `if ( (int) $wco->get_meta( WC_SCANPAY_URI_SHOPID, true, 'edit' ) <= 0 ) { $wco->add_meta_data( WC_SCANPAY_URI_SHOPID, $this->shopid, true ); }`
   — only when absent, never overwriting: a *different* shop id is the mismatch
   `scheduled_charge()` already refuses at `:117-129`, and it must stay refused.
   The existing `save_meta_data()` call persists both.
2. Do the same in the `$paid_renewal` block of `generate-payment-link.php`, which
   likewise writes only `WC_SCANPAY_URI_COMPLETE` — the shop id there is
   `(int) strstr( (string) $settings['apikey'], ':', true )`, as at `:257`.
3. While in `charge()`: keep the client's return value and log it. Every other
   outcome of a renewal — invalid subid, bad key, shop mismatch, amount mismatch,
   zero amount, already paid, any throw — writes a log line; the one that moves
...
it, and this task exists so that allowance stops costing the merchant an
unreconcilable order.

### Verify

- Trace a charge on an order with no stamp end to end, and state where it lands
  today: `sync():258` returns, `capture():55` throws, and the retry short-circuit
  at `:291` writes no status. Quote all three.
- Confirm the stamp is written before `$this->client->charge()` and after the
  already-paid guard, so it is never written for an order this call declines to
  charge.
- Confirm the `!== $this->shopid` mismatch path is untouched: construct the three
  cases (absent, equal, different) on paper and report what each does now.
- Confirm `add_meta_data( …, true )` replaces rather than appends
  (`abstract-wc-data.php`, `add_meta_data()`'s `$unique` branch).

### Handoff

- Only a 1.x-migrated store settles this one. On one: find a Scanpay subscription
  whose renewal orders carry no `_scanpay_shopid`, let a renewal charge, and
  confirm the order now reaches `processing`/`completed` after the ping instead
  of logging "shopid mismatch".
- Confirm the new `info` line appears in the WooCommerce log (source
  `wc-scanpay`) with the charge id, and that it does not appear for skipped or
  failed renewals.

## Task D — Two ways one change can wedge the whole sync loop

**File:** `src/library/class-wc-scanpay-sync.php`, `sync()` at `:245-253` and
`:274-288`.

The drain loop's contract is that a local anomaly logs and returns while a
backend protocol violation throws. A throw out of `sync()` reaches
`wc-scanpay-ping.php:354`, answers 500 and leaves the cursor where it was — so
the same seq page replays on every ping and **no order in the shop syncs again**.
Two lines break that contract in the direction of the outage.

**1. `wc_get_order()` can hand back something that is not a `WC_Order`.**
...
Refunds share the order id space, and `wc_get_order()` resolves a refund id to a
`WC_Order_Refund`, which extends `WC_Abstract_Order` and defines none of
`get_transaction_id()`, `set_transaction_id()`, `get_payment_method()` or
`payment_complete()` — those are `WC_Order`'s, and nothing in the chain defines
`__call`. So a change whose `orderid` names a refund is not the logged skip the
comment at `:247-249` promises; it is an undefined-method `Error`, which
`wc-scanpay-ping.php:354` catches as a `Throwable` — so not a white-screen fatal,
but the 500-and-replay above, which is the shop-wide sync outage. The codebase already guards the identical call site the right way:
`library/functions.php:34`, `! $wco instanceof \WC_Order`, with the comment "or a
refund rather than an order".

**2. The underpayment branch calls `add_order_note()` unguarded**, under a
comment that reads "Log + note + return, never throw". `add_order_note()` runs
`woocommerce_new_order_note_data`, `wp_insert_comment()` and
`woocommerce_order_note_added` (`class-wc-order.php:2080-2146`) — all
third-party surface. `report_incomplete()` fifty lines below wraps the very same
call in `try`/`catch ( \Throwable )`, with the rule spelled out: "Reporting a
failure must not become the failure … the cursor advances either way."

**The fix.**

1. Change the guard at `:246` to `if ( ! $wco instanceof WC_Order )`, keeping the
   same `warning` log and return. Extend the existing comment with the refund
   case, citing `functions.php`'s twin.
2. Wrap the underpayment `add_order_note()` in the same
   `try`/`catch ( \Throwable )` + `scanpay_log( 'error', … )` shape as
   `report_incomplete():381-394`.

**Do not** also try to make the underpayment note idempotent. It is re-added on
every later revision of the same transaction, because the branch returns before
`set_transaction_id()` and so leaves the `:253` gate open — noise, not harm, and
...
- Read `includes/class-wc-order-refund.php:17` and `includes/class-wc-order.php:898`
  and confirm in `HANDOFF.md` that `get_transaction_id()` is `WC_Order`-only and
  that no `__call` exists on `WC_Order_Refund`, `WC_Abstract_Order`,
  `WC_Abstract_Legacy_Order` or `WC_Data`.
- Confirm both guards keep the cursor advancing: trace each to its `return` and
...
- Not reproducible without a shop and a colliding id. If one is available:
  create a refund, then have Scanpay deliver a change whose `orderid` is that
  refund's id, and confirm the ping answers 200 with a `warning` line instead of
  500.

## Task E — `update_status()` reports failure by return value, not by throwing

**Files:** `src/woocommerce-scanpay.php`, `wcs_scanpay_fail_renewal()` at
`:320-333` (as task B left the file); `src/library/class-wc-scanpay-capture.php`,
`capture_or_hold()` at `:159-174`.

Both sites wrap `update_status()` in `try`/`catch ( \Throwable )` and treat the
absence of a throw as success. `WC_Order::update_status()`
(`class-wc-order.php:402-426`) catches `Exception` itself — it logs through
`wc_get_logger()`, adds its own "Update status event failed." note, and
**returns `false`**. Our `catch` therefore only fires for an `Error` out of a
third-party callback; every ordinary failure returns quietly.

What each one then gets wrong:

- **`wcs_scanpay_fail_renewal()`** is the one place in the renewal flow that
  writes `'failed'`. Its docblock says the failure is "contained here and
  appended to the same log entry … including when the status write is itself
  what fails" — the *containment* half is true and stays true; what the code
  cannot do is **notice**, because the ordinary failure never throws. If the
  write fails, the order stays `pending`, so
  `WC_Subscriptions_Renewal_Order::maybe_record_subscription_payment()` never
  calls `payment_failed()`, `woocommerce_subscription_renewal_payment_failed`
  never fires, and `WCS_Retry_Manager` schedules no retry — the subscription is
  stranded, and our log says the renewal was marked failed.
- **`capture_or_hold()`** promises, in its docblock, that any error parks the
  order `on-hold`. If that write fails the order keeps the status it had —
  `completed`, on the `woocommerce_order_status_completed` path — while our log
  says only "Capture on order #N failed", which reads as "parked" to anyone
  following the documented contract. WooCommerce does leave its own trail (a
  `wc_get_logger()` line and an "Update status event failed." note on the order),
  but nothing ties it to ours, and neither says the order is still `completed`.
  The merchant ships against an authorization that was never captured.

**The fix.** Keep both `try`/`catch` blocks — they still contain a non-`Exception`
`Throwable` — and add the return-value check inside each, the same way the
...
- Confirm the WCS chain from a `failed` renewal order to a scheduled retry:
  ...
  line for each hop, so the consequence of a missed status write is on record.
- Confirm no caller of either function reads a return value that changes shape.
