...
## Task A: Migrate the released 2.x SQL schema before stamping version 3

`upgrade.php:12` gates on versions below `2.0.0`, and `install.php` creates a
table only when it is entirely absent (`SHOW TABLES LIKE` at `:12`, `:29`,
`:53`), never inspecting an existing definition. A 2.9.1 site matches no branch
and reaches `upgrade.php:111`, which stamps `3.0.0` after doing nothing.

The schema really does differ (`git show v2.9.1:src/install.php`):

- `scanpay_meta.method VARCHAR(64) NOT NULL` — **no DEFAULT** — while the v3
  insert (`class-wc-scanpay-sync.php:230`) no longer supplies `method`.
- `scanpay_subs` still carries `retries`, `nxt`, `method_id` and `idem`, which
  v3's lock-free charge design never reads.

Do not confuse the tables: `scanpay_subs.method` is legitimate in v3
(`install.php:58`, written at `class-wc-scanpay-sync.php:346-351`). Only
`scanpay_meta.method` is the orphan. Because it is `NOT NULL` with no default, a
strict SQL mode fails the insert outright (MySQL 1364) instead of degrading to
`''`. WordPress strips those modes on its own connection
(`.stubs/wordpress/wp-includes/class-wpdb.php:644-651`, applied at `:975`), but
...
1. Add an idempotent, version-gated v2→v3 migration in `upgrade.php` before the
   final version update, running for every installed version below `3.0.0`.
   Require `install.php` for the whole `< 3.0.0` branch, not only the existing
   `< 2.0.0` one, so a missing table is created from the v3 schema. This cannot
   stamp the version early: existing settings or an existing version make
   `install.php`'s `$fresh_install` false. Keep `install.php` the single source
   of the fresh v3 schema.

   A pre-2.0.0 site then requires `install.php` twice (it is already required at
   `upgrade.php:17`). Harmless, since it is idempotent — and **not** an
   invitation to restructure the `< 2.0.0` branch to avoid the apparent
   duplicate, which would break the 1.x legacy-table drop at `:14-15`.

2. Inspect before altering: `SHOW COLUMNS` with the trusted prefixed table name
   and a prepared `LIKE`. Drop `scanpay_meta.method` and the four obsolete
   `scanpay_subs` columns only when present. Do not rely on
   `DROP COLUMN IF EXISTS` — keep compatibility with the oldest MySQL/MariaDB
   the WordPress minimum supports.

   **Put the inspection in a new `src/library/schema.php`**: one prefixed
   function returning a table's column names, `require_once`'d from
   `upgrade.php`. Task B needs the same read from the reset endpoint, and neither
   file can `require` the other. It does **not** belong in
   `library/functions.php`, which the card gateway loads on every front-end
   request — both callers here are admin-only, and the plugin stays cheap to load
   by keeping admin-only code out of the always-loaded modules.

3. Preserve every live row and cursor. Do not drop/recreate any table, do not
   reset `seq`/`ping`/`mtime`/totals/revisions/method data, and do not add
   retry, lock or idempotency state back to `scanpay_subs`.

4. Treat every schema read or write failure as an upgrade failure: check
   `$wpdb->last_error` after inspection, check each `ALTER TABLE` result and
   throw descriptively, and leave `wc_scanpay_version` unchanged unless the
   complete migration and all older ones succeed. Check the final
   `update_option()` and its reread too — this branch starts from a different
   version, so a failed write is not an unchanged-value no-op. Preserve the
   loader's five-minute throttle and last-write version stamp.
   ...
   `false ===` is the correct test, matching `install.php`'s existing
   `true !== $res` idiom; `if ( ! $res )` would be wrong on an
...
- Diff `git show v2.9.1:src/install.php` against `src/install.php` and enumerate
  ...
  assumed. The released schema also carries redundant UNIQUE constraints v3
  dropped in `5f4468b`: harmless and out of scope, but say so rather than
  leaving it unexplained.
- Every `ALTER` is guarded by a presence check.
- No code path stamps the version before the migration returns.
- `install.php` still produces exactly the v3 schema for a fresh install.
- The column read lives in `src/library/schema.php` and nothing on a front-end
  request requires it.
...
## Task B: Make reset exclusive with sync, and check its destructive operations

`admin/hooks/wp-ajax-wc-scanpay-reset.php` clears the key, drops three tables
(`:57-59`), recreates them (`:61-69`) and reports success (`:71-72`) — with no
...
on `SHOW TABLES LIKE`, a failed drop means the surviving table is found,
creation is skipped, and success is still returned.

### Fix

1. Read and retain the old shop ID **before** touching settings — after `:52`
   unsets `apikey` it is unrecoverable.

2. If it is valid, acquire that shop's `Scanpay_Flock` and hold it across the
   whole reset. Load `library/class-scanpay-flock.php` with `require_once`
   first: the admin bootstrap loads the gateway classes but not the flock —
   `callback/wc-scanpay-ping.php:203` is its only `require` in `src/`.
   ...
   - Release explicitly on every path *before* `wp_send_json_*()`; do not rely
     on the destructor after `exit`.

   The class already supports this. `acquire()` is non-blocking (`LOCK_EX |
   LOCK_NB`, `class-scanpay-flock.php:41`), returns `false` on contention and
   throws `RuntimeException` when the file cannot be opened (`:36-40`) — exactly
   the three-way distinction needed. `release()` is public and idempotent
   (`:51-57`). A second `acquire()` on a held lock returns `false`, so do not
   double-acquire.

   Scope, stated honestly: the ping worker re-reads the cursor under every
   acquire (`wc-scanpay-ping.php:262`, loop at `:245`/`:341`, handoff re-read at
   `:336`) and exits 500 once the `scanpay_seq` row is gone (`:53-57`), so a
   worker re-acquiring after reset fails safe. But the heartbeat and
   busy-handoff writes at `:197` and `:227-231` happen **without** the lock;
   racing a reset they either hit a dropped table (500, self-heals via the
   keepalive) or the recreated empty table (a 200 acking a stranded ping). Both
   acceptable — the data was deliberately deleted — but do not claim the lock
   covers every writer. `Scanpay_Flock` only coordinates workers sharing a
   lock-file filesystem; document that multi-node deployments need
   `WP_TEMP_DIR` on shared storage, and do not introduce a second
   distributed-lock design.

3. Check each drop's result and stop on `false`. They are already three separate
   statements (`:57-59`) — keep them that way. Do not consolidate into a
   multi-table `DROP`: MySQL DDL atomicity is per table even in 8.0.
...
4. Do not return success merely because `install.php` did not throw. Verify: the
   ...
   v3 column set** (the column-inspection function in `src/library/schema.php`,
   `require_once`'d here too), they hold no old rows, and no sequence row was
   seeded while the key is empty.

   **Existence alone is not enough, and Task A makes that concrete.** The
   failure model is precisely that a failed `DROP` leaves a survivor which
   `SHOW TABLES LIKE` then finds, so creation is skipped. After A lands, that
   survivor can be a *2.x-shaped* table, so an existence-only postcondition lets
   a "successful" reset silently reintroduce the strict-mode insert failure A
   exists to remove — on a site that had already been migrated. A precedes this
   task, so this is live, not hypothetical.

   Note `DROP TABLE IF EXISTS` returns `true` for an **absent** table, so a
   truthy drop result is not evidence of deletion; only the postcondition is.
   Item 3 is necessary but not sufficient. For `create|alter|truncate|drop`,
   `$wpdb->query()` returns `$this->result`, effectively `true`, never a row
   count (`class-wpdb.php:2308-2309`), so `false === $res` is the correct test —
   matching `install.php`'s existing `true !== $res` idiom; `if ( ! $res )` would
   be wrong on an `insert|delete|update` query. And do not treat a bare `false`
   from `update_option()` as
   proof of failure — WordPress also returns `false` when the value is already
   stored; check the reread instead. Database `DROP`, `CREATE`/inspection and
   postcondition failures remain real failures.
...
6. Preserve the existing guard: `current_user_can( 'manage_woocommerce' )` plus
   `check_ajax_referer( 'wc-scanpay-reset', 'nonce', false )` (`:26-31`). There
   is **no shared-secret check on this endpoint** — the secret guards the
   `?x=meta|ping|sub` fetch endpoints. Do not add one while "preserving" it.
   Also preserve gateway disabling (`:41-55`) and `install.php` behaviour: reset
   does not re-stamp a version (`install.php:77` requires **both** an absent
   settings option and an absent `wc_scanpay_version`) and keeps the existing
   secret (`:92`).

### Verify

- Trace every exit path and confirm each releases the lock before responding.
- No `wp_send_json_*()` precedes a release, and no path double-acquires.
- The old shop ID is read before `:52`.
- The flock class is loaded on this admin path before construction.
- Each `DROP` result is captured and short-circuits.
- Unchanged settings cannot be misreported as a persistence failure; the reread,
  not `update_option()`'s boolean alone, decides it.
...
## Task C: Remove Scanpay data from every site during a multisite uninstall
...
1. Extract the existing per-blog cleanup into a local function: drop all current
   ...
   plugin file being defined. Keep the literal option names local to the helper.

   **The helper carries six drops** — the three current tables plus three legacy
   ones: `scanpay_queue`, `woocommerce_scanpay_queuedcharges` and
   `woocommerce_scanpay_seq`. Every one must move *inside* the helper. Leaving
   `scanpay_queue` at top level means a network uninstall drops it on one blog
   and leaves it on every other — precisely the gap this task closes, reopened
   for one table. Carry its comment block, which explains why it must not be
   deleted again, along with it.

2. Single-site: call it once, behaviour unchanged.

3. Multisite: request site IDs rather than `WP_Site` objects and run the helper
   for every blog that could hold data.

   - **Paginate explicitly.** `get_sites()` defaults to `'number' => 100`
     ...
     advance the offset until a short page.
...
4. Derive table names from `$wpdb->prefix` only *after* the switch, or the
   helper repeatedly targets the original site's tables.
...
6. **Per-order meta is deliberately out of scope.** Uninstall leaves
   ...
   the unbounded order sweep item 5 forbids.

### Verify

- Every `switch_to_blog()` is paired with a `restore_current_blog()` in a
  `finally`, including the failure path.
- The prefix is read inside the loop, not hoisted.
- Pagination cannot stop at the default 100-site boundary and requests IDs
  rather than objects.
- Enumerate every option the plugin writes and confirm the helper covers each —
  ...
  (`admin/settings/process-admin-options.php:79`), `wc_scanpay_version` and the
