 ...
 *
 *  $order_id is the third argument WooCommerce and WCS have always passed
 *  (class-wc-order.php:1896, class-wc-subscriptions-order.php:1203, documented
 *  '@param int $order_id'). It predates the WC 3.6 minimum, so no fallback.
 */
function wc_scanpay_item_needs_processing( bool $needs_processing, \WC_Product $product, int $order_id ): bool {
	...
	return false; // Our order, and the product is virtual but not downloadable.
}
